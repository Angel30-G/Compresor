...
static void on_comprimir_serial(GtkWidget *widget, gpointer data) {
    ...
    char comando[2048];
    snprintf(comando, sizeof(comando), "bin/compresor_serial \"%s\" /tmp/salida_comprimida", directorio_seleccionado);
    char salida[8192] = {0};
...
static void on_comprimir_parallel(GtkWidget *widget, gpointer data) {
    if (!directorio_seleccionado) { g_print("Seleccione un directorio primero.\n"); return; }
    char comando[2048];
    snprintf(comando, sizeof(comando), "bin/compresor_parallel \"%s\" /tmp/salida_comprimida_par", directorio_seleccionado);
    char salida[8192] = {0};
    if (ejecutar_comando(comando, salida, sizeof(salida))) {
        Stats stats = {0};
        parsear_stats(salida, &stats);
...
static void on_comprimir_concurrent(GtkWidget *widget, gpointer data) {
    if (!directorio_seleccionado) { g_print("Seleccione un directorio primero.\n"); return; }
    char comando[2048];
    snprintf(comando, sizeof(comando), "bin/compresor_concurrent \"%s\" /tmp/salida_comprimida_conc", directorio_seleccionado);
    char salida[8192] = {0};
    if (ejecutar_comando(comando, salida, sizeof(salida))) {
        Stats stats = {0};
        parsear_stats(salida, &stats);
...
static void on_descomprimir_serial(GtkWidget *widget, gpointer data) {
    char comando[2048];
    snprintf(comando, sizeof(comando), "bin/descompresor_serial /tmp/salida_comprimida /tmp/salida_descomprimida_serial");
    char salida[8192] = {0};
    if (ejecutar_comando(comando, salida, sizeof(salida))) {
        Stats stats = {0};
        parsear_stats(salida, &stats);
...
static void on_descomprimir_parallel(GtkWidget *widget, gpointer data) {
    char comando[2048];
    snprintf(comando, sizeof(comando), "bin/descompresor_parallel /tmp/salida_comprimida_par /tmp/salida_descomprimida_par");
    char salida[8192] = {0};
    if (ejecutar_comando(comando, salida, sizeof(salida))) {
        Stats stats = {0};
        parsear_stats(salida, &stats);
...
static void on_descomprimir_concurrent(GtkWidget *widget, gpointer data) {
    char comando[2048];
    snprintf(comando, sizeof(comando), "bin/descompresor_concurrent /tmp/salida_comprimida_conc /tmp/salida_descomprimida_conc");
    char salida[8192] = {0};
    if (ejecutar_comando(comando, salida, sizeof(salida))) {
        Stats stats = {0};
        parsear_stats(salida, &stats);
